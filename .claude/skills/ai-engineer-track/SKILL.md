...
- Đặt 2-4 câu hỏi hoặc 1 mini task để xác nhận hiểu bài, tập trung vào **lý thuyết, áp dụng thực tế, và các điểm cần lưu ý (trade-off/khi nào không dùng)** — KHÔNG hỏi theo kiểu "trace code" (vd "dòng X in ra giá trị gì", "biến Y ở bước Z bằng bao nhiêu", "hàm này follow nhánh nào") vì đó là kiểm tra đọc code chứ không phải hiểu khái niệm. Câu hỏi đúng tinh thần: "Nó là gì / vì sao cần / khi nào KHÔNG dùng / áp dụng vào tình huống mới thế nào". Ví dụ dạng câu hỏi: "Nếu dữ liệu có nhiều mã số/tên riêng, chọn hybrid search hay vector thuần? Vì sao?" hoặc "Cho 1 trường hợp KHÔNG nên fine-tune mà chỉ cần RAG, giải thích lý do." Có thể tham chiếu tới hiện tượng đã quan sát được khi chạy bài thực hành (vd "bạn vừa thấy X xảy ra khi chạy code — điều đó minh họa nguyên lý gì, áp dụng sang tình huống Y thì sao") thay vì hỏi lại chi tiết vận hành nội bộ của đoạn code.
